...
class QuizzyScreen:
    ...
    def initialize(self):
        ...
        go_back_button = tkinter.Button(self.frame, text="Return to homescreen", command=self.handle_back)
    ...
    def initialize_question_view(self):
        ...
        if quizzy_service.check_for_more_questions(q_num, self.questions):
            ...
            self.question_view = QuestionView(self.question_frame, question, self.initialize_question_view)
            self.question_view.pack()
        else:
            result = tkinter.Label(self.question_frame, text=f"You got {points}/{len(self.questions)} right!", fg="black", bg="white")
            result.pack()
            quizzy_service.update_user_highscore(points)
            end = tkinter.Label(self.question_frame, text=f"Please return to homescreen", fg="black", bg="white")
            end.pack()
...
class QuestionView:
    ...
    def initialize(self):
        ...
        q = tkinter.Label(self.frame, text=f"{q_num+1}. {self.question.question}", fg="black", bg="white", padx=200, pady=20)
        q.pack()
        var = tkinter.StringVar()
        var.set(self.question.options[0])
        for opt in self.question.options:
            tkinter.Radiobutton(self.frame, text=opt, variable=var, value=opt, fg="black", bg="white", pady=5).pack()
        self.submit = tkinter.Button(self.frame, text="Submit", command=lambda: self.handle_submit(var.get()), fg="white", bg="white", bd=0)
        self.submit.pack()
    ...
    def handle_submit(self, value):
        ...
        if not self.answerd:
            if quizzy_service.check_if_correct(self.question, value):
                points += 1
                right_answer = tkinter.Label(self.frame, text=f"You got a point!", fg="black", bg="white")
                right_answer.pack()
            else:
                wrong_answer = tkinter.Label(self.frame, text=f"Answer was wrong\nRight answer was {self.question.answer}", fg="black", bg="white")
                wrong_answer.pack()
            self.submit.destroy()
            self.next = tkinter.Button(self.frame, text="Next question", command = self.handle_next, fg="white", bg="white", bd=0)
            self.next.pack()
        self.answerd = True
